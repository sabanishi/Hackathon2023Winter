    ...
    public class JumpRampEntity:BaseEntity
    {
        ...
        private void Run()
        {
            ...
            UniTask.Void(async () =>
            {
                ...
                //床のY座標が止まるまで待機
                await floor.StopObservable.ToUniTask(cancellationToken:this.GetCancellationTokenOnDestroy(),useFirstValue:true);
                //waitTime秒待機
        ...
        protected override void ChangeToOfflineInternal()
        {
            ...
            if (floorGameObject.GetComponent<PhotonTransformView>() != null)
            {
                Destroy(floorGameObject.GetComponent<PhotonTransformView>());
            }
            if(floorGameObject.GetComponent<PhotonRigidbody2DView>() != null)
            {
                Destroy(floorGameObject.GetComponent<PhotonRigidbody2DView>());
            }
